...
function loadCsvAsObjects(filePath: string): {
  headers: string[];
...
async function main() {
  ...
  try {
    ...
    // ─── 1. LCDs ────────────────────────────────────────────────────────
    const lcdRows = loadCsvAsObjects(
      path.join(args["lcd-dir"], "lcd.csv"),
    ).rows;
    ...
    for (const r of aContractorRows) {
      ...
      const c = contractorById.get(r.contractor_id);
      if (!c) continue;
      aContractorValues.push([dbId, c.number || r.contractor_id, c.name, null]);
    }
    ...
    for (const r of hcpcRows) {
      ...
      hcpcValues.push([
        ...
        nonEmpty(r.hcpc_code_group) ? Number(r.hcpc_code_group) || null : null,
      ]);
    }
