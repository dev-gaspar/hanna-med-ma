...
export class RpaService {
  ...
  async markPatientAsSeen(
    ...
    if (needsRegistration) {
      ...
      this.processRpaRegistrationAsync(
        patient.id,
        doctorId,
        patient.name,
      ).catch((err) => {
        this.logger.error(
          `Background RPA task failed for ${patientId}: ${err.message}`,
        );
      });
    }
    ...
    this.logger.log(
      `Encounter ${encounter.id} created for patient ${patientId} by doctor ${doctorId}.`,
    );
   ...
   */
  private async processRpaRegistrationAsync(
    patientId: number,
    doctorId: number,
    patientName: string,
  ) {
    this.logger.log(
      `[Background RPA] Starting for patient ${patientId} (${patientName})`,
    );
    ...
    if (patient.rawData.length > 0) {
      ...
      try {
        this.logger.log(
          `[Background RPA] Formatting payload for ${patientName}...`,
        );
        const aiOutput =
          await this.subAgentsService.formatCareTrackerInsurancePayload(
            latestInsuranceRaw.rawContent,
        ...
        this.logger.log(
          `[Background RPA] Pushing CareTracker task for ${patientName} to Redis...`,
        );
        ...
        this.logger.error(
          `[Background RPA] Payload Prep Failed: ${err.message}`,
        );
        await this.prisma.patient.update({
          where: { id: patientId },
      ...
      this.logger.warn(
        `[Background RPA] Patient ${patientId} has no INSURANCE raw data. Generating dummy without RPA.`,
      );
  ...
  async handleCareTrackerResult(dto: CaretrackerResultDto) {
    ...
    this.logger.log(
      `Received RPA result for patient ${patientId}: success=${success}, status=${status}`,
    );
  ...
  async getPatientDataStatus(
    ...
    Record<string, { summary: boolean; insurance: boolean; lab: boolean }>
  > {
    const node = await this.prisma.rpaNode.findUnique({
      where: { uuid },
      select: { doctorId: true },
    ...
    const result: Record<
      string,
      { summary: boolean; insurance: boolean; lab: boolean }
    > = {};
  ...
  async updateEncounterNote(
    ...
    if (data.providerNote !== undefined)
      updateData.providerNote = data.providerNote;
    if (data.noteAttempts !== undefined)
      updateData.noteAttempts = data.noteAttempts;
  ...
  private normalizeCareTrackerPayload(payload: unknown): unknown {
    ...
    if (body.search_query) {
      body.search_query.first_name = (body.search_query.first_name || "")
        .toLowerCase()
        .trim();
      body.search_query.last_name = (body.search_query.last_name || "")
        .toLowerCase()
        .trim();
    }
