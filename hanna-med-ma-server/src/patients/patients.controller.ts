...
export class PatientsController {
  ...
  @ApiOperation({
    summary: "List patients on the authenticated doctor's census",
  })
  ...
  @ApiBearerAuth("JWT-auth")
  @ApiOperation({
    summary: "Get IDs of patients the current doctor has marked as seen",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary:
      "Get full patient detail — encounters, raw-data timeline, presigned PDF URLs",
  })
  @ApiParam({ name: "id", type: "number" })
  async detail(@Param("id", ParseIntPipe) patientId: number, @Request() req) {
    const doctorId = req.user.userId;
