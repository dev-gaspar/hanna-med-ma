...
class BaseBatchSummaryFlow(BaseFlow, ABC):
    ...
    def notify_completion(self, result):
        """Send consolidated results to the backend ingestion endpoint."""
        payload = {
            "status": "completed",
        ...
        logger.info("[BATCH-SUMMARY] Sending results to backend...")
        response = self._send_to_summary_webhook_n8n(payload)
        if response:
            logger.info(f"[BATCH-SUMMARY] Backend response: {response.status_code}")
        else:
            logger.error("[BATCH-SUMMARY] Failed to send to backend")
