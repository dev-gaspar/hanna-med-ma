...
class JacksonBatchInsuranceFlow(BaseFlow):
    ...
    def _extract_insurance(self) -> str:
        ...
        if not location:
            ...
            if more_alt_img:
                logger.info(
                    "[JACKSON-BATCH-INS] Trying alternate 'More' button image..."
                )
                location = self.wait_for_element(
                    more_alt_img,
                    timeout=10,
        ...
        if not location:
            ...
            if ins_alt_img:
                logger.info(
                    "[JACKSON-BATCH-INS] Trying alternate 'Insurance Information' image..."
                )
                location = self.wait_for_element(
                    ins_alt_img,
                    timeout=10,
        ...
        if not location:
            ...
            if guar_alt_img:
                logger.info(
                    "[JACKSON-BATCH-INS] Trying alternate 'Guarantors' image..."
                )
                location = self.wait_for_element(
                    guar_alt_img,
                    timeout=10,
    ...
    def notify_completion(self, result):
        """Send consolidated insurance results to backend."""
        payload = {
            "status": "completed",
            "type": self.FLOW_TYPE,
            "doctor_name": self.doctor_name,
        ...
        logger.info(
            f"[BACKEND] Batch insurance notification sent - Status: {response.status_code}"
        )
        return response
