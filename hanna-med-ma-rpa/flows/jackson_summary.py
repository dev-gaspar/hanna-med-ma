...
class JacksonSummaryFlow(BaseFlow):
    ...
    def _cleanup_with_patient_detail_open(self):
        ...
        try:
            ...
            logger.info("[JACKSON SUMMARY] Waiting 15s for system to process close...")
            stoppable_sleep(15)
    ...
    def notify_completion(self, result):
        """Notify backend of completion with the copied content or patient-not-found status."""
        ...
        logger.info(
            f"[BACKEND] Summary notification sent - Status: {response.status_code}"
        )
        return response
