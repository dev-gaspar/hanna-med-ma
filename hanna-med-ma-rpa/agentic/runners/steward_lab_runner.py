"""
Steward Lab Runner - Local orchestrator for patient lab results extraction.

Follows the same pattern as StewardInsuranceRunner for batch compatibility:
1. PatientFinderAgent - Find patient in list (with scroll support)
2. Click on patient to open detail
...
Lab extraction phases are handled by the flow, not the runner.
"""
...
class LabRunnerResult:
    ...
    patient_element_id: Optional[int] = None


class StewardLabRunner:
    """
    Local orchestrator for Steward patient lab results flow.
    ...
    Same pattern as StewardInsuranceRunner but for lab extraction.
    """
    ...
    def run(self, patient_name: str) -> LabRunnerResult:
        ...
        Returns:
            LabRunnerResult with outcome including patient_element_id
        """
        ...
        logger.info("=" * 70)
        logger.info(" STEWARD LAB RUNNER - STARTING")
        ...
        try:
            ...
            if patient_result.status == "not_found" or patient_result.status == "error":
                logger.warning(
                    "[LAB-RUNNER] Patient not found in Rounds Patients list"
                )
            ...
            logger.info(
                f"[LAB-RUNNER] Phase 1 complete - Patient at element {patient_element_id}"
            )

            # === PHASE 2: Click Patient (RPA) ===
            logger.info("[LAB-RUNNER] Phase 2: Clicking on patient...")
            self._phase2_click_patient(patient_element_id, phase1_elements)
            logger.info("[LAB-RUNNER] Phase 2 complete - Patient clicked")

            logger.info("=" * 70)
            logger.info(" STEWARD LAB RUNNER - FINISHED")
    ...
    def _phase1_find_patient_with_scroll(self, patient_name: str):
        ...
        if using_roi:
            ...
            logger.warning(
                "[LAB-RUNNER] No ROI configured for steward patient_finder"
            )
        ...
        for step in range(1, MAX_PATIENT_STEPS + 1):
            ...
            logger.info(
                f"[LAB-RUNNER] Phase 1 Step {step}/{MAX_PATIENT_STEPS} - {scroll_state}"
            )
            ...
            if result.status == "not_found":
                logger.warning(
                    "[LAB-RUNNER] Patient not found after searching entire list"
                )
            ...
            if result.status == "running":
                if result.action == "scroll_down":
                    logger.info("[LAB-RUNNER] Scrolling DOWN in patient list...")
                    ...
                    logger.info("[LAB-RUNNER] Scrolling UP in patient list...")
                    ...
                    logger.info("[LAB-RUNNER] Waiting for screen to stabilize...")
        ...
        logger.warning(
            f"[LAB-RUNNER] Phase 1 exhausted {MAX_PATIENT_STEPS} steps without finding patient"
        )
    ...
    def _phase2_click_patient(self, element_id: int, elements: list):
        ...
        Args:
            ...
            elements: Elements list from Phase 1
        """
        self.current_step += 1
        ...
        logger.info(f"[LAB-RUNNER] Clicked patient element {element_id}")
        ...
        try:
            ...
            if modal:
                logger.info("[LAB-RUNNER] Confidentiality modal detected - clicking Yes")
