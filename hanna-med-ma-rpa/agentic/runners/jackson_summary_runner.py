...
class JacksonSummaryRunner:
    ...
    def _phase2_open_patient_and_notes(self, element_id: int, elements: list):
        ...
        # Try to click 'By Type' to sort notes
        self._click_by_type_sort()

    def _click_by_type_sort(self):
        """Click 'By Type' to sort notes if visible."""
        logger.info("[RUNNER] Checking for 'By Type' sort option...")
        try:
            by_type_image = config.get_rpa_setting("images.jackson_by_type")
            if by_type_image:
                location = pyautogui.locateOnScreen(by_type_image, confidence=0.8)
                if location:
                    self.rpa.safe_click(location, "By Type (Sort)")
                    logger.info("[RUNNER] Clicked 'By Type' sort option")
                    self._record_step(
                        "rpa", "click_by_type", "Clicked By Type sort option"
                    )
                    self.rpa.stoppable_sleep(2)
                else:
                    logger.info("[RUNNER] 'By Type' sort option not found on screen")
        except Exception as e:
            logger.warning(f"[RUNNER] Error checking for 'By Type': {e}")
